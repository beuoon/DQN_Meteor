let DQN = function () {
	...
	// Constant
	this.CHANGE_TARGET_NETWORK_INTERVAL = 50;
	
	this.REPLAY_DATA_MAX = 5000;
	this.IMAGE_SIZE = {width: 50, height: 50}; // 학습에 사용될 이미지 크기
	this.STATE_SIZE = 4; // 학습에 사용될 이미지 개수
	
	this.REWARD_GAMMA = 1; // 0.9; // 미래 보상값에 대한 gamma 값
	
	this.MINIMUM_EPSILON = 0.01; // 임의 행동의 최소 epsilon 값
	this.EPSILON_BETA = 0.6; // 임의 행동 epsilon 값의 감소율
}
DQN.prototype = {
    init: function (croppingSize) {
		...
		this.network.addLayer(new Layer_Linear(layerSize, 250));
    ...
    train: function() {
		...
		for (let i = 0; i < replayDatas.length; i++) {
			...
			let futureReward = this.targetNetwork.forward(replayData.nextState);
